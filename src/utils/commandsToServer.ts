...
const initializeSocket = () => {
    if (!socket && getCommandPromptStore().TERMINAL_MODE === 'server') {
        ...
        socket.on('output', (data: string) => {
            ...
            getCommandPromptStore().createNewPromptInstanceAndDisablePreviousInstance(
                PROMPT_INSTANCE
            )
        })
...
const sendInputToServer = (input: string, promptInstance: PromptInstance) => {
    if (getCommandPromptStore().TERMINAL_MODE !== 'server') {
        console.warn(
            'Terminal is not in server mode. No command will be sent to the server.'
        )
        return
    }
    ...
    getCommandPromptStore().handleCommandInputEnterServer(
        promptInstance.id,
        input,
        async () => socket?.emit('command', input)
    )
}
