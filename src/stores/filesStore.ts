...
export const useFilesStore = defineStore('filesStore', () => {
  ...
  const initFromLocalStorage = () => {
    const storedTerminalFiles = localStorage.getItem(
      'BROWSER_TERMINAL_FILES'
    )
  ...
  const deleteFile = (fileName: string, path?: string[]): boolean => {
    const map = findMapAtPath(path)
  ...
  const deleteFolder = (folderName: string, path?: string[]): boolean => {
    const parentMap = findMapAtPath(path)

    if (parentMap && parentMap.has(folderName)) {
      const folder = parentMap.get(folderName)
      if (folder instanceof Map) {
        parentMap.delete(folderName)
        saveToLocalStorage()
        return true
      }
    }
