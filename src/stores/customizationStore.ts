...
export const useCustomizationStore = defineStore('customizationStore', () => {
    // Default values
    const TERMINAL_BACKGROUND_COLOR = ref(
        themes.darkMode.TERMINAL_BACKGROUND_COLOR
    )
    const TERMINAL_REPLY_COLOR = ref(themes.darkMode.TERMINAL_REPLY_COLOR)
    const TERMINAL_DIRECTORY_PROMPT_COLOR = ref(
        themes.darkMode.TERMINAL_DIRECTORY_PROMPT_COLOR
    )
    ...
    const updateCustomization = (customizationData: Record<string, string>) => {
        if (customizationData.TERMINAL_BACKGROUND_COLOR) {
            TERMINAL_BACKGROUND_COLOR.value =
                customizationData.TERMINAL_BACKGROUND_COLOR
        }
        ...
        if (customizationData.TERMINAL_DIRECTORY_PROMPT_COLOR) {
            TERMINAL_DIRECTORY_PROMPT_COLOR.value =
                customizationData.TERMINAL_DIRECTORY_PROMPT_COLOR
        }
        if (customizationData.TERMINAL_COMMAND_PROMPT_COLOR) {
            TERMINAL_COMMAND_PROMPT_COLOR.value =
                customizationData.TERMINAL_COMMAND_PROMPT_COLOR
        }
        if (customizationData.TERMINAL_LS_FILE_COLOR) {
            TERMINAL_LS_FILE_COLOR.value =
                customizationData.TERMINAL_LS_FILE_COLOR
        }
        ...
        if (customizationData.TERMINAL_DEFAULT_THEME) {
            TERMINAL_DEFAULT_THEME.value =
                customizationData.TERMINAL_DEFAULT_THEME as keyof typeof themes
        }
        saveToLocalStorage()
    }
    ...
    const saveToLocalStorage = () => {
        ...
        localStorage.setItem(
            'BROWSER_TERMINAL_CUSTOMIZER',
            JSON.stringify(customizationJson)
        )
    }

    // Initialization: Check if customization settings exist in localStorage
    const initFromLocalStorage = () => {
        const storedCustomization = localStorage.getItem(
            'BROWSER_TERMINAL_CUSTOMIZER'
        )
